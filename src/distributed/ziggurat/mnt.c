...
double ziggurat_mnt_generate(ziggurat_mnt_t *cache with_gc(gc)) {
    ...
    if (cache->with_bottom_offset) {
        while (true) {
            double gen = rand_gen(gen_call(gc));
            ...
            gen -= bp;
            gen /= 1 - bp;
            if (ziggurat_mnt_try_generate(&result, gen pass_gc(gc), cache)) {
                return result;
            }
        }
    }
...
ziggurat_mnt_err_t ziggurat_mnt_init(ziggurat_mnt_t *result, ziggurat_mnt_config_t *config) {
    ...
    if (DISTMOD_ZIGGURAT_IS_RIGHT != is_right) {
	LOGE("cannot create %s function because of DISTMOD_ZIGGURAT_DISABLE_DYNAMIC_DIRECTION", is_right ? "left" : "right");
	...
	LOGE("cannot create from not adjoining function because of DISTMOD_ZIGGURAT_DISABLE_BOTTOM_OFFSET");
